<script lang="ts">
        ...
        if (data) {
          ...
          for (const category of CATEGORIES) {
            ...
            if (category.subcategories) {
              const subcategory = category.subcategories.find((sub: AvitoCategory) => sub.deeplink === deeplink);
              if (subcategory) {
                selectedCategory = category;
  ...
  async function loadCategoryItems(category: AvitoCategory, subcategory?: AvitoCategory, page: number = 1) {
    loading = true;
    error = null;
...
<div class="md:container md:mx-auto">
  <header class="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border hidden md:block">
  ...
  <main class="p-2 md:p-4 pb-20 md:pb-4">
    {#if error}
    ...
    <BreadcrumbHeader 
      title={selectedCategory ? selectedSubcategory?.name || selectedCategory.name : `Объявления в ${$selectedCity.name}`}
    ...
    <ItemsList 
      title={selectedCategory ? selectedSubcategory?.name || selectedCategory.name : $query ? "Результаты поиска" : `Объявления в ${$selectedCity.name}`}
