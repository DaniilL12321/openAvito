<script lang="ts">
				...
				: searchQuery
					? {
							...
							verticalCategoryId: urlParams.get('verticalCategoryId')
								...
								: undefined,
							pmin: urlParams.get('pmin')
								? parseInt(urlParams.get('pmin')!)
								: undefined,
							pmax: urlParams.get('pmax')
								? parseInt(urlParams.get('pmax')!)
								: undefined,
	...
	function handleScroll() {
		...
		if (window.innerHeight + window.scrollY >= scrollHeight - 1000 && !loading && hasMore) {
			loadMore();
		}
	}
...
<section class="space-y-4 md:space-y-6">
	...
	<div
		class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-3 md:gap-4 lg:grid-cols-4 xl:grid-cols-5"
	>
		{#each items.filter((item) => !item.bannerId && item && item.title && item.urlPath && item.priceDetailed?.string && (item.images?.length > 0 || item.locationId)) as item, index (`${item.id}-${Math.floor(index / 50) + 1}`)}
			<ItemCard {item} />
		{/each}
	</div>
	...
	{#if !hasMore && !error && isSearch && recommendations.length > 0}
		<div class="pt-8">
			<h3 class="mb-4 text-lg font-semibold">Вам также может понравиться</h3>
			<div
				class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-3 md:gap-4 lg:grid-cols-4 xl:grid-cols-5"
			>
				{#each recommendations.filter((item) => !item.bannerId && item && item.title && item.urlPath && item.priceDetailed?.string && (item.images?.length > 0 || item.locationId)) as item, index (`rec-${item.id}-${index}`)}
					<ItemCard {item} />
				{/each}
			</div>
		</div>
