...
>
	<div
		...
		class="fixed bottom-0 z-50 h-[85vh] w-full overflow-y-auto rounded-t-3xl border bg-background p-0 shadow-lg duration-200 md:bottom-auto md:left-[50%] md:top-[50%] md:h-auto md:max-h-[90vh] md:min-h-[400px] md:w-[calc(100vw-2rem)] md:max-w-6xl md:translate-x-[-50%] md:translate-y-[-50%] md:rounded-3xl"
	...
	>
		<div class="sticky top-0 z-30 border-b bg-background/80 backdrop-blur-sm">
			<div class="px-4 pb-2 pt-4 md:px-6 md:pb-4 md:pt-6 lg:px-8 lg:pb-6 lg:pt-8">
				<div class="relative flex items-center justify-between gap-2">
					<div class="flex items-center gap-2 md:gap-3">
						<Heart class="h-5 w-5 flex-shrink-0 text-primary md:h-6 md:w-6" />
						<div>
							<h2
							...
							>
								...
								{#if activeCount > 0}
									<p class="text-xs text-muted-foreground md:text-sm">
										{activeCount}
										{activeCount === 1 ? 'активное' : 'активных'}
									</p>
								{/if}
							</h2>
						</div>
					...
					<div class="flex items-center gap-2">
						{#if inactiveCount > 0}
							<button
								class="inline-flex flex-shrink-0 items-center gap-1.5 rounded-full bg-secondary px-2 py-1.5 text-xs font-medium text-secondary-foreground hover:bg-secondary/80 md:gap-2 md:px-3 md:text-sm"
		...
		<div class="p-4 lg:p-8">
			<div class="space-y-6">
				...
				<div
					class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-3 md:gap-4 lg:grid-cols-4"
				>
					{#each filteredFavorites as item (item.id)}
						...
						>
							<a
								...
								on:click|preventDefault={() => (selectedItem = item)}
								class="block"
							>
								<div class="relative aspect-square overflow-hidden">
									...
									{:else}
										<img
											...
											alt={item.imagesAlt}
											loading="lazy"
											class="h-full w-full object-cover transition-transform group-hover:scale-105 {!item.isActive &&
												'opacity-50'}"
											on:error={() => handleImageError(item.id)}
										/>
										{#if !item.isActive}
											<span
												class="absolute left-2 top-2 rounded-full bg-secondary px-3 py-1 text-xs font-medium text-secondary-foreground"
											>
												Закрыто
											</span>
										{/if}
								...
								<div class="p-4">
									<h3
										class="line-clamp-2 text-sm font-medium {(!item.isActive ||
											imageErrors[item.id] ||
											item.isDeleted) &&
									...
									<p
										class="mt-2 text-lg font-bold {(!item.isActive ||
											imageErrors[item.id] ||
											item.isDeleted) &&
											'text-muted-foreground'}"
									>
					...
					<div class="py-4 text-center text-sm text-muted-foreground">
						{#if $favorites.length > 0}
							{$showInactiveItems
								...
								: 'Все избранные объявления закрыты'}
						{:else}
							У вас пока нет избранных объявлений
						{/if}
					</div>
