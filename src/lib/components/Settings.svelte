<script lang="ts">
  ...
  import { Settings as SettingsIcon, X, Sun, Moon, Smartphone, Cookie, Trash2, Plus } from 'lucide-svelte';
...
<button
  class="inline-flex items-center justify-center rounded-full w-10 h-10 hover:bg-primary/10 dark:hover:bg-primary/20 text-foreground transition-colors"
  on:click={handleOpen}
>
...
{#if showModal}
  <div use:portal>
    ...
    >
      <div
        class="fixed bottom-0 z-50 h-[85vh] w-full overflow-y-auto border bg-background p-0 shadow-lg duration-200 rounded-t-3xl md:bottom-auto md:left-[50%] md:top-[50%] md:h-auto md:max-h-[90vh] md:w-[calc(100vw-2rem)] md:max-w-lg md:translate-x-[-50%] md:translate-y-[-50%] md:rounded-3xl"
        ...
        <div class="sticky top-0 bg-background/80 backdrop-blur-sm border-b z-30">
          <div class="px-4 py-3 md:px-6 md:py-4">
            <div class="relative flex items-center justify-between gap-2">
              <h2 class="text-lg font-semibold leading-tight tracking-tight md:text-2xl">Настройки</h2>
              <button
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-3 gap-2">
                <button
                  class="flex flex-col items-center gap-2 p-3 rounded-xl border transition-colors
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-2 gap-2">
                {#each colorSchemes as scheme}
                  ...
                  >
                    <div class="w-6 h-6 rounded-full shadow-sm" style="background: hsl({scheme.primary})"></div>
              ...
              {#if showColorPicker}
                <div class="mt-4 space-y-6" transition:fly={{ y: 20, duration: 200, opacity: 1, easing: cubicOut }}>
                  <!-- Оттенок -->
                  <div class="space-y-2">
                    <label class="text-sm font-medium">Оттенок</label>
                    <div class="h-11 md:h-8 rounded-lg relative overflow-hidden">
                ...
                <div class="space-y-2">
                  ...
                  <div class="space-y-3">
                    <textarea
                      ...
                      class="w-full h-24 px-3 py-2 text-sm rounded-xl border resize-none bg-background"
                      placeholder="Вставьте cookies здесь..."
                    />
                    <div class="relative group">
                      <button
                        class="w-full flex items-center justify-center gap-2 p-3 rounded-xl transition-colors {cookiesChanged ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'bg-muted text-muted-foreground cursor-not-allowed'}"
                      ...
                      {#if !cookiesChanged}
                        <div 
                          class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 rounded-xl bg-popover text-popover-foreground text-xs font-medium shadow-lg pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border"
                        >
                          Измените cookies для активации кнопки
                          <div class="absolute top-full left-1/2 -translate-x-1/2 -translate-y-[1px] border-8 border-transparent border-t-popover"></div>
                        </div>
                      {/if}
                    </div>
                  </div>
