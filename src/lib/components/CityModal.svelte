<script lang="ts">
	...
	function selectCity(city: (typeof cities)[0]) {
		selectedCity.set({
			...
			name: city.names[1],
			url:
				city.names[1] === 'Все регионы'
					? ''
					: city.names[1].toLowerCase().replace(/[^а-яё-]/g, '')
		});
...
>
	<div
		class="fixed bottom-0 z-[70] h-[85vh] w-full overflow-y-auto rounded-t-3xl border bg-background p-0 shadow-lg duration-200 md:bottom-auto md:left-[50%] md:top-[50%] md:h-auto md:max-h-[90vh] md:w-[calc(100vw-2rem)] md:max-w-xl md:translate-x-[-50%] md:translate-y-[-50%] md:rounded-3xl"
	...
	>
		...
		<div class="p-4 md:p-6">
			...
			<div class="space-y-2">
				{#if loading}
					<div class="px-3 py-2 text-sm text-muted-foreground">Загрузка...</div>
				{:else if cities.length === 0}
					<div class="px-3 py-2 text-sm text-muted-foreground">Ничего не найдено</div>
				{:else}
					{#each cities as city}
						<button
							class="w-full rounded-2xl p-3 text-left transition-colors hover:bg-primary/10 dark:hover:bg-primary/20 {$selectedCity.id ===
							city.id
