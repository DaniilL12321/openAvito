<script lang="ts">
	...
	let groupedSubcategories: {
		[key: string]: { header?: AvitoCategory; items: AvitoCategory[] };
	} = {};
		...
		if (currentCategory) {
			dispatch('select', {
				category: {
					...
					deeplink:
						category.deeplink ||
						`ru.avito://1/items/search?categoryId=${category.categoryId}&microcategoryId=${category.microcategoryId}&presentationType=serp&sort=date`
				}
			});
			close();
		} else {
...
>
	<div
		class="fixed bottom-0 z-[70] h-[85vh] w-full overflow-y-auto rounded-t-3xl border bg-background p-0 shadow-lg duration-200 md:bottom-auto md:left-[50%] md:top-[50%] md:h-auto md:max-h-[90vh] md:w-[calc(100vw-2rem)] md:max-w-2xl md:translate-x-[-50%] md:translate-y-[-50%] md:rounded-3xl"
	...
	>
		...
		<div class="space-y-4 p-4 pb-20 md:pb-4">
			{#if loading}
				...
				{#if groupedSubcategories['auto']}
					<div class="grid grid-cols-2 gap-3">
						{#each groupedSubcategories['auto'].items as category}
							<button
								class="flex w-full items-center justify-between rounded-lg border border-border px-4 py-3 text-left text-sm transition-colors hover:bg-primary/10 hover:text-primary dark:hover:bg-primary/20"
								on:click={() => selectCategory(category)}
							>
								<span class="truncate">{category.name}</span>
								<ChevronLeft
									class="h-4 w-4 flex-shrink-0 rotate-180 text-muted-foreground"
								/>
							</button>
						{/each}
					</div>
				...
				{#each Object.entries(groupedSubcategories) as [groupId, group]}
					{#if groupId !== 'ungrouped' && groupId !== 'auto'}
						<div class="space-y-3">
							{#if group.header}
								<button
									class="w-full rounded-lg bg-muted px-4 py-2 text-left transition-colors hover:bg-primary/10 hover:text-primary dark:hover:bg-primary/20"
