...
<div class="relative w-full" bind:this={selectContainer}>
	...
	{#if isOpen}
		<div
			class="absolute left-0 top-[calc(100%+4px)] z-50 w-full rounded-lg border bg-popover text-popover-foreground shadow-md"
			transition:fly={{ y: -5, duration: 150, opacity: 1, easing: cubicOut }}
		>
			<div class="p-1">
				{#each options as option}
					<button
						class="relative flex w-full cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-primary/10 hover:text-primary data-[selected=true]:bg-accent/50 dark:hover:bg-primary/20"
