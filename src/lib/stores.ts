...
const savedShowInactive =
	typeof window !== 'undefined' ? localStorage.getItem(SHOW_INACTIVE_KEY) : null;

export const avitoCookies = writable<string>(savedCookies || '');
export const showInactiveItems = writable<boolean>(
	savedShowInactive !== null ? savedShowInactive === 'true' : true
);
